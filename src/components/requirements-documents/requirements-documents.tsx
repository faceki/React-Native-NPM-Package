...
const RequirementsDocuments = ({
  ...
  return (
    <>
            ...
            documentSteps.map((item, index) => {
              return (
                <View key={index} style={[styles.box, { marginTop: 5 }]}>
                  ...
                  {item.includes("selfie") ? <Image style={styles.tinyLogo} source={require(SELFIE)} /> : <Image style={styles.tinyLogo} source={require(IDCARDBACK)} />}
                  <Text>{item}</Text>
                </View>
              )
      ...
      <View>
        {consenttermofuseLink && <Pressable onPress={handleClick}>
        ...
        <View style={styles.row}>
          ...
          />
          {consenttermofuseLink && <Text style={styles.subHeading}>
